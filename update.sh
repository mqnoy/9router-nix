#!/usr/bin/env bash
#
# update.sh — Automatically update 9router to the latest (or specified) npm version.
#
# Usage:
#   ./update.sh              # update to latest version on npm
#   ./update.sh 0.4.66       # update to a specific version
#   ./update.sh --check      # just print latest version without updating
#
set -euo pipefail

REPO_DIR="$(cd "$(dirname "$0")" && pwd)"
NIX_FILE="$REPO_DIR/9router.nix"
LOCK_DIR="$REPO_DIR/packages/9router"
NPM_PACKAGE="9router"
...
# ── Get current version from 9router.nix ──────────────────────────────────
get_current_version() {
...
TARBALL_URL="$NPM_REGISTRY/$NPM_PACKAGE/-/9router-$TARGET_VERSION.tgz"
...
curl -sL "$TARBALL_URL" -o "$TEMP_DIR/9router.tgz"
mkdir -p "$TEMP_DIR/extract"
tar -xzf "$TEMP_DIR/9router.tgz" -C "$TEMP_DIR/extract"
...
# ── Step 4: Update version + source hash in 9router.nix ──────────────────
step "Updating 9router.nix (version + source hash)"
...
step "Setting npmDepsHash in 9router.nix"
...
if RESULT="$(nix-build "$REPO_DIR" 2>&1)"; then
  ...
  echo ""
  echo -e "${GREEN}${BOLD}✓ Updated 9router: $CURRENT_VERSION → $TARGET_VERSION${NC}"
  echo ""
  echo "  Files changed:"
  echo "    • 9router.nix  (version, hash, npmDepsHash)"
  echo "    • packages/9router/package-lock.json"
  echo ""
  echo "  Don't forget to commit:"
  echo "    git add 9router.nix packages/9router/package-lock.json"
  echo "    git commit -m \"9router: $CURRENT_VERSION → $TARGET_VERSION\""
else
